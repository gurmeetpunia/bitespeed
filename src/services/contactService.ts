...
import { Contact, IdentifyRequest, ContactResponse } from '../types/database';

export class ContactService {
  // Find contacts by email or phone number
  private findContactsByEmailOrPhone(email?: string, phoneNumber?: string): Promise<Contact[]> {
  ...
  private getAllLinkedContacts(primaryId: number): Promise<Contact[]> {
    return new Promise((resolve, reject) => {
      const sql = `
        SELECT * FROM Contact 
      ...
      db.all(sql, [primaryId, primaryId], (err, rows: any[]) => {
        if (err) {
          reject(err);
        } else {
          const contacts = rows.map(row => ({
  ...
  private createContact(
    ...
    return new Promise((resolve, reject) => {
      ...
      db.run(sql, [email, phoneNumber, linkedId, linkPrecedence], function(err) {
        if (err) {
          reject(err);
        } else {
          resolve(this.lastID);
  ...
  private updateContactToSecondary(contactId: number, linkedId: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const sql = `
        UPDATE Contact 
      ...
      db.run(sql, [linkedId, contactId], (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
  ...
  private updateLinkedContacts(oldPrimaryId: number, newPrimaryId: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const sql = `
        UPDATE Contact 
      ...
      db.run(sql, [newPrimaryId, oldPrimaryId], (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
  ...
  async identify(request: IdentifyRequest): Promise<ContactResponse> {
    ...
    if (contactGroups.size > 1) {
      ...
      const hasExactMatch = allLinkedContacts.some(contact => 
    ...
    const hasExactMatch = groupContacts.some(contact => 
      contact.email === email && contact.phoneNumber === phoneNumber
    );
